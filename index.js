import{a as L,T as G,S as I,N as K,A as U}from"./assets/vendor-BeY3HugT.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&o(d)}).observe(document,{childList:!0,subtree:!0});function s(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(n){if(n.ep)return;n.ep=!0;const r=s(n);fetch(n.href,r)}})();const E="https://deserts-store.b.goit.study/api";async function J(){const{data:e}=await L.get(`${E}/categories`);return e}async function D({page:e,limit:t,category:s}){const o={page:e,limit:t};s&&s!=="all"&&(o.category=s);const{data:n}=await L.get(`${E}/desserts`,{params:o});return n}const p=document.querySelector(".dessert-filters"),a=document.querySelector(".dessert-filters__list"),c=document.getElementById("dessert-dropdown-btn"),u=document.getElementById("selected-category-name"),m=document.querySelector(".dessert-gallery"),y=document.querySelector("#loader"),l=document.querySelector(".dessert-list__load-more-btn");let k="all",g=1;const b=8;async function O(){q(),v(),g=1,m&&(m.innerHTML="");try{const e=await D({page:g,limit:b,category:k});F(e.desserts||[]),e.totalItems>b?w():v()}catch(e){console.error(e)}finally{$()}}async function Q(){g+=1,q(),v();try{const e=await D({page:g,limit:b,category:k}),t=e.desserts||[];F(t),(m?m.children.length:0)>=e.totalItems||t.length<b?v():w()}catch(e){console.error(e),w()}finally{$()}}async function W(){q();try{const e=await J();Y(e),await O()}catch(e){console.error(e)}finally{$()}}function Y(e){if(!a)return;let t=`
    ...
    `;t+=e.map(({_id:s,name:o})=>`
      ...
      `).join(""),a.innerHTML=t}function F(e){if(!m)return;const t=e.map(({_id:s,name:o,description:n,price:r,category:{name:d},image:B})=>`
